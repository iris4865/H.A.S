...
namespace HatchlingNet
{
    ...
    {
        public delegate void CompletedMessageCallback(byte[] buffer);//나중에 메세지가 문자열로 안오고 클래스로 오면 바껴야 되는데
                                                                    //한번에 어떻게 못하나?
        ...
        public void Translate(byte[] buffer, int offset, int transffered, CompletedMessageCallback completeMessageCallback)
        {
            ...
            while (this.remainBytes > 0)
            {
                ...
                if (complete)
                {
                    completeMessageCallback(this.messageBuffer);
                    ClearBuffer();
                }
